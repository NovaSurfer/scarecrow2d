...
namespace sc2d::memory
{
    ...
    public:
        ...
        constexpr pointer
        allocate(size_type n, typename std::allocator<void>::const_pointer hint = nullptr) const;

        constexpr pointer deallocate(pointer p, size_type size) const;

        constexpr void construct(pointer p, const_reference value) const
        {
            new(p) value_type(value);
        }

        constexpr void destroy(pointer p) const
        {
            p->~value_type();
        }
        ...
        constexpr bool operator==(const aligned_allocator&) const
        {
            return true;
        }

        constexpr bool operator!=(const aligned_allocator& rhs) const
        {
            return !operator==(rhs);
        }
    };
    ...
    aligned_allocator<T, N>::allocate(size_type n,
                                      typename std::allocator<void>::const_pointer hint) const
    {
