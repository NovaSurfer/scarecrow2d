...
default_tex2d::default_tex2d(const GLuint width, const GLuint height, const GLuint internal_format,
...
void texgen::generate(T* texdata)
{
    ...
    if constexpr(std::is_same<T, default_tex2d>::value) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, texdata->internal_format, texdata->width, texdata->height, 0,
                     texdata->image_format, GL_UNSIGNED_BYTE, texdata->data);
    }
    // TEXTURE 2D ARRAY
    if constexpr(std::is_same<T, default_tex2darr>::value) {
        texture_type = GL_TEXTURE_2D_ARRAY;

        const uint32_t bytes_per_channel = texdata->image_format == GL_RGB ? 3 : 4;
        ...
        for(size_t i = 0; i < tiles_length; ++i) {
            ...
            glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, tile_width, tile_height, 1, GL_RGBA,
                            GL_UNSIGNED_BYTE, texdata->data + ptr_offset * bytes_per_channel);
        }
    }
